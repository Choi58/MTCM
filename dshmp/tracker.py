...
import spacy
from .modeling.transformer_decoder.modules import ReferringTracker_RVOS
...
class Tracker(nn.Module):
    ...
    def train_model(self, batched_inputs, iterations):
        ...
        outputs_tracker['pred_logits'] = rearrange(outputs_tracker['pred_logits'], '(l b t) q c -> l b t q c', b=B, t=T).mean(dim=2) # b q c -> l b q c
        outputs_tracker['pred_masks'] = rearrange(outputs_tracker['pred_masks'], '(l b t) q h w -> l b q t h w', b=B, t=T)
        for i, aux_outputs in enumerate(outputs_tracker['aux_outputs']):
            outputs_tracker['aux_outputs'][i]['pred_logits'] = rearrange(aux_outputs['pred_logits'], '(l b t) q c -> l b t q c', b=B, t=T).mean(dim=2)
            outputs_tracker['aux_outputs'][i]['pred_masks'] = rearrange(aux_outputs['pred_masks'], '(l b t) q h w -> l b q t h w', b=B, t=T)
        ...
        vita_loss_dict = self.vita_criterion(outputs_tracker, clip_targets, frame_targets, fg_indices[-1:])
    ...
    def inference(self, batched_inputs):
        ...
        vita_outputs, vita_frame_embds, vita_video_embds = self.vita_module(frame_queries, lang_feat_fusion, lang_mask_, 
                                                                        motion_feat, return_frame_query=True)

        #vita_frame_embds = rearrange(vita_frame_embds, 'l b t q c -> (l b) c t q')
        #vita_video_embds = repeat(vita_video_embds, 'l b q c -> (l b) t q c', t=num_frames)
        ...
        mask_cls = outputs_tracker["pred_logits"].mean(dim=0) # cQ, K+1
        mask_pred = outputs_tracker["pred_masks"].transpose(0,1)   # cQ, t, h, w
